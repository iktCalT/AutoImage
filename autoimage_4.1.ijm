...
// Version 4.1 Add parameter "remove_undesired_images"
...
remove_undesired_images = paraset[36]; // "Y" or "N", for safety, it will only delete images when remove_undesired_images is "Y" 
...
if (! File.isDirectory(backup_path + "/undesired")) File.makeDirectory(backup_path+"/undesired");
        ...
        if (endsWith("" + file_list[i], "" + original_extension)) {
            ...
            if(!File.exists(input_path + file_list[i])){
                ...
                if(roinum != 0) {
                    ...
                    if (remove_undesired_images != "Y") {
                    	File.copy(input_path + filename, backup_path + "/undesired/" + filename);
                    }
                }
